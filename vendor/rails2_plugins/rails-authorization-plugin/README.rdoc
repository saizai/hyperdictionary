...
You can also pass an arbitrary non-string to be used instead of an authorization exprssion.

E.g.: permit MyModel.special_holiday_permissions?(Time.now) do ... 

==== Options

  :allow_guests => false.

We can allow permission processing without a current user object. The default is false. Only used for hardwired roles; see Special below if using object table.
...
Role queries follow the same pattern as the setting of roles:
  ...
  user.roles_for Group # all roles on any groups (or Group itself)
  user.roles_for group # all roles on this group
  user.roles_for Group, 'moderator' # moderator roles on any Groups
  user.roles_for group, ['moderator', 'member'] # member OR moderator roles on this group

  a_model.has_roles 'moderator' # all moderator users of this model; may also include AnonUser (see below)
  a_model.has_roles [Role.find_by_name('moderator'), 'admin', 'members'] # accepts arrays, Roles, and plurals too
  
  # hash of all model's users w/ their roles: {'some_role' => [user1, AnonUser, user2], 'other_role' => [user2, user3], ...}
  a_model.users_by_roles 
...
These require the ObjectRolesTable mixin.
...
E.g. permit 'admin or (self of user)' for Users#show_private_info
...
Equivalent to login_required
  ...
  AnonUser.has_role 'editor of public_wiki_post'
  ...
  AnonUser.has_no_role 'editor of locked_wiki_post'
  permit 'editor of public_wiki_post' # Allows anon users, or anyone else explicitly assigned the role
  permit 'guest or (editor of locked_wiki_post)' # Does the same, but doesn't require assigning AnonUser the role
  AnonUser.has_role? 'foo'
  Bar.accepts_role 'foo', AnonUser
  Bar.accepts_role 'foo', nil # equivalent
  AnonUser.is_admin?  # hopefully false ;-)
